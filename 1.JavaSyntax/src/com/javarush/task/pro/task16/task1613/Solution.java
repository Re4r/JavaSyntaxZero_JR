...
public class Solution {
    ...
    public static boolean isWeekend(LocalDateTime dateTime) {
        ...
        if (SATURDAY == dayValue) return true;
        if (FRIDAY == dayValue && dateTime.getHour() >= WEEKEND_START_FRIDAY_CUT_OFF_HOUR) return true;
        if (SUNDAY == dayValue && dateTime.getHour() < WEEKEND_END_SUNDAY_CUT_OFF_HOUR) return true;
